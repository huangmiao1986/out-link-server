package com.out.link.server.http.redis.impl;
...
import com.out.link.server.http.redis.JedisBuilder;
...
//	public Logger loggerInfo = LoggerFactory.getServerInfoLogger(FriendListServiceImpl.class);
	public Logger logger = LoggerFactory.getServerErrorLogger(RedisOperatorImpl.class);
	
	private JedisBuilder jedisBuilder;
	
	public JedisBuilder getJedisBuilder() {
		return jedisBuilder;
	}

	public void setJedisBuilder(JedisBuilder jedisBuilder) {
		this.jedisBuilder = jedisBuilder;
	}
	...
	public Long expire(String key, int seconds, int db){
		...
		try{
			...
			flag = false;
			logger.error("redis call expire(key="+key+", time="+seconds + ", db="+db+") exception: "+e.toString());
			jedisBuilder.returnBrokenResource(jedis);
		}finally{
			if(flag)
				jedisBuilder.returnResource(jedis);
	...
	public String hmset(String key, int db,  Map<String, String> kvals){
		...
		try{
			...
			flag = false;
			logger.error("redis call hmset(key="+key+", kvals="+kvals+"db="+db+") exception: "+e.toString());
			jedisBuilder.returnBrokenResource(jedis);
		}catch(Exception e){
			flag = false;
			logger.error("redis call hmset(key="+key+", kvals="+kvals+"db="+db+") exception: "+e.toString());
	...
	public Long zcard(String key, int db) {
		...
		try{
			...
			flag = false;
			logger.error("redis call zcard(key="+key+",db="+db+") exception: "+e.toString());
			jedisBuilder.returnBrokenResource(jedis);
		}finally{
			if(flag)
				jedisBuilder.returnResource(jedis);
	...
	public Long ttl(String key, int db) {
		...
		try{
			...
			flag = false;
			logger.error("redis call zrangeWithScores(key="+key+", db="+db+") exception: "+e.toString());
			jedisBuilder.returnBrokenResource(jedis);
		}catch(Exception e){
			flag = false;
			logger.error("redis call zrangeWithScores(key="+key+", db="+db+") exception: "+e.toString());
	...
	public String auth(String password, int db) {
		...
		try{
			...
			flag = false;
			logger.error("redis call auth exception: "+e.toString());
			jedisBuilder.returnBrokenResource(jedis);
		}catch(Exception e){
			flag = false;
			logger.error("redis call auth exception: "+e.toString());
